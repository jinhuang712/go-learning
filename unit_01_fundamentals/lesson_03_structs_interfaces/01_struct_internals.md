...
2. **对象体积很大**时。如果使用“值接收者”，每次调用方法都会把整个庞大的结构体在内存中完整拷贝一份；而使用“指针接收者”，每次调用只拷贝一个 8 字节的指针地址，从而**避免了值拷贝的性能开销**。
...
在微服务开发中，我们经常需要临时组装一些数据（例如构造一个复杂的 JSON 响应给前端）。
如果这个数据结构只在这一个函数里用到，专门去外面写一个 `type XxxResponse struct {...}` 会让代码显得非常臃肿。

此时，我们可以直接在代码执行处“**就地声明类型 + 就地初始化赋值**”，这就是匿名结构体。
你可以把它理解为把普通的 `Type{赋值}` 中的 `Type`，替换成了一大坨完整的结构体定义。

```go
// 拆解来看，前面这部分是“类型定义” (也就是它长什么样)：
// struct {
//     Code int    `json:"code"`
//     Msg  string `json:"msg"`
// }
...
// 现在你可以直接把它序列化成 JSON 返回给前端
// jsonBytes, _ := json.Marshal(response)
```
**Java 对比**：这有点像在 Java 中为了拼装一个一次性的复杂 JSON 结果，不去建一个 DTO 类，而是直接 `new HashMap<String, Object>()` 往里塞数据。但 Go 的匿名结构体比 Map 更好，因为它有**强类型约束**和**编译期检查**。
...
> **疑问：结构体还能包含 Func？**
> 是的！不要把“方法 (Method)”和“字段 (Field)”搞混。
> - **方法**是绑定在类型外面的（`func (u User) Do()`）。
> - **字段**是结构体内部的数据。因为 Go 中的函数是“一等公民”，所以你可以定义一个字段的类型为 `func()`，用来存储一个回调函数 (Callback)。
> ```go
> type Task struct {
>     Name     string
>     Callback func(err error) // 这是一个字段，存放一个函数指针
> }
> ```
